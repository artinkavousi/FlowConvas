...
capsule pipeline rather than inventing structure. The full 11-step pipeline, the two conversion modes,
and the final report format are the single source of truth in **`docs/converter-pipeline.md`**; the
module/Lab contract is **`docs/module-and-lab-standards.md`**.

> **Blueprint first (step 7, mandatory).** Before writing any module/Lab code, write
> `docs/conversions/<id>-conversion-plan.md` (Overview section first) — granular enough for a blind downstream agent. Template:
> `docs/templates/blueprint.template.md`; spec: `docs/blueprinting.md`. The shapes below are what that
> blueprint plans toward and what execution then builds.

## Module folder contract

```
STUDIO/src/modules/<category>/
  <Feature>.tsx    # self-contained runtime/component source
...
`AuroraShader.js` (untyped Three.js owns the loop + dispose):
...
import { useBridgeStore } from '@artinos/panelflow';
import { createThing } from './AuroraShader';
...
```
rendering/postfx/WebgpuSsgiRoomRenderer.js
rendering/environments/AdaptiveOpenFrontBoxRoom.js
physics/particles/UniversalPhysicsParticleSystem.js
physics/particles/BounceRigidSphereAdapter.js
input/PointerGlassCollider.js
labs/ball-pool/...
```
...
| `*.ts(x)` runtime + `*.showcase.tsx` wrapper | Generic rewrite that only resembles the source |
