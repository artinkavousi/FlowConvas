...
> Status: **IN PROGRESS** — 9 modules built & gated (check-registry + lint green); 10 modules + Lab pending.
...
- [ ] **T2.1** `physics/fluid/mls-mpm-solver` · mirror: `mls-mpm.ts` kernels verbatim + pointrenderer render
  pattern · out-of-scope: boundaries/postfx/audio · acceptance: solver point cloud animates, gravity-center
  pulls particles into a sphere, 0 console errors.
- [ ] **T2.2** `physics/particles/particle-boundaries` · acceptance: box/sphere deflects a point cloud.
