...
#### [~] T-14 (M): External conversion proof — WebGPU Fluid Simulation *(GATED → BLOCKED)*
> **BLOCKED (2026-06-22):** The `WebGpu-Fluid-Simulation-master` source named in ARTINOS-PRD §17 is **not present anywhere in the repo** (verified by search). Per ADR-9 / root AGENTS.md §4, no generic substitute will be invented. **Unblock:** drop the source folder into the repo (or name an available Three.js/R3F/ShaderToy source) and re-run this task. The registry→showcase loop is already proven by the 3 ported seed modules; this task only covers the *external* conversion half of FR-16.
