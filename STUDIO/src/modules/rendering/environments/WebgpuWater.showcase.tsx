...
import { createWebgpuWaterScene } from './WebgpuWater.module';

const BRIDGE_ID = 'webgpu-water-mesh';

export default function WebgpuWaterShowcase() {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const engineRef = useRef<ReturnType<typeof createWebgpuWaterScene> | null>(null);
  const values = useBridgeStore((s) => s.componentValues[BRIDGE_ID]);

  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    const engine = createWebgpuWaterScene(canvas, {});
