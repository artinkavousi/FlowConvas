...
import { createTslHashBlurScene } from './TslHashBlur.module';

const BRIDGE_ID = 'tsl-hash-blur';

export default function TslHashBlurShowcase() {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const engineRef = useRef<ReturnType<typeof createTslHashBlurScene> | null>(null);
  const values = useBridgeStore((s) => s.componentValues[BRIDGE_ID]);

  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    const engine = createTslHashBlurScene(canvas, {});
