...
import { createTslGtaoScene } from './TslGtao.module';

const BRIDGE_ID = 'tsl-gtao';

export default function TslGtaoShowcase() {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const engineRef = useRef<ReturnType<typeof createTslGtaoScene> | null>(null);
  const values = useBridgeStore((s) => s.componentValues[BRIDGE_ID]);

  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    const engine = createTslGtaoScene(canvas, {});
