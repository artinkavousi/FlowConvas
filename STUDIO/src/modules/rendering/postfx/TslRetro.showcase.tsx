...
import { createTslRetroScene } from './TslRetro.module';

const BRIDGE_ID = 'tsl-retro';

export default function TslRetroShowcase() {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const engineRef = useRef<ReturnType<typeof createTslRetroScene> | null>(null);
  const values = useBridgeStore((s) => s.componentValues[BRIDGE_ID]);

  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    const engine = createTslRetroScene(canvas, {});
