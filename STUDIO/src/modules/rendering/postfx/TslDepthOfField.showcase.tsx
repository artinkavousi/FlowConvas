...
import { createTslDofScene } from './TslDepthOfField.module';

const BRIDGE_ID = 'tsl-depth-of-field';

export default function TslDepthOfFieldShowcase() {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const engineRef = useRef<ReturnType<typeof createTslDofScene> | null>(null);
  const values = useBridgeStore((s) => s.componentValues[BRIDGE_ID]);

  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    const engine = createTslDofScene(canvas, {});
