...
import { createTslFsr1Scene } from './TslFsr1.module';

const BRIDGE_ID = 'tsl-fsr1';

export default function TslFsr1Showcase() {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const engineRef = useRef<ReturnType<typeof createTslFsr1Scene> | null>(null);
  const values = useBridgeStore((s) => s.componentValues[BRIDGE_ID]);

  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    const engine = createTslFsr1Scene(canvas, {});
