...
import { createTslSobelScene } from './TslSobel.module';

const BRIDGE_ID = 'tsl-sobel';

export default function TslSobelShowcase() {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const engineRef = useRef<ReturnType<typeof createTslSobelScene> | null>(null);
  const values = useBridgeStore((s) => s.componentValues[BRIDGE_ID]);

  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    const engine = createTslSobelScene(canvas, {});
