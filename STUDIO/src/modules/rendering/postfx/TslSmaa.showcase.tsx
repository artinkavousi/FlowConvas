...
import { createTslSmaaScene } from './TslSmaa.module';

const BRIDGE_ID = 'tsl-smaa';

export default function TslSmaaShowcase() {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const engineRef = useRef<ReturnType<typeof createTslSmaaScene> | null>(null);
  const values = useBridgeStore((s) => s.componentValues[BRIDGE_ID]);

  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    const engine = createTslSmaaScene(canvas, {});
