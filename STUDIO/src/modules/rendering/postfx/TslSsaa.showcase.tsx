...
import { createTslSsaaScene } from './TslSsaa.module';

const BRIDGE_ID = 'tsl-ssaa';

export default function TslSsaaShowcase() {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const engineRef = useRef<ReturnType<typeof createTslSsaaScene> | null>(null);
  const values = useBridgeStore((s) => s.componentValues[BRIDGE_ID]);

  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    const engine = createTslSsaaScene(canvas, {});
