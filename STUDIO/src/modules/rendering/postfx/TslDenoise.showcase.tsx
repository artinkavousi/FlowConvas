...
import { createTslDenoiseScene } from './TslDenoise.module';

const BRIDGE_ID = 'tsl-denoise';

export default function TslDenoiseShowcase() {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const engineRef = useRef<ReturnType<typeof createTslDenoiseScene> | null>(null);
  const values = useBridgeStore((s) => s.componentValues[BRIDGE_ID]);

  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    const engine = createTslDenoiseScene(canvas, {});
