...
import { createTslAfterImageScene } from './TslAfterImage.module';

const BRIDGE_ID = 'tsl-after-image';

export default function TslAfterImageShowcase() {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const engineRef = useRef<ReturnType<typeof createTslAfterImageScene> | null>(null);
  const values = useBridgeStore((s) => s.componentValues[BRIDGE_ID]);

  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    const engine = createTslAfterImageScene(canvas, {});
