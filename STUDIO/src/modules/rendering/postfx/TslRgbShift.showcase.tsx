...
import { createTslRgbShiftScene } from './TslRgbShift.module';

const BRIDGE_ID = 'tsl-rgb-shift';

export default function TslRgbShiftShowcase() {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const engineRef = useRef<ReturnType<typeof createTslRgbShiftScene> | null>(null);
  const values = useBridgeStore((s) => s.componentValues[BRIDGE_ID]);

  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    const engine = createTslRgbShiftScene(canvas, {});
