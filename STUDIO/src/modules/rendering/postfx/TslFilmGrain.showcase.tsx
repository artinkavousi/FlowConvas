...
import { createTslFilmScene } from './TslFilmGrain.module';

const BRIDGE_ID = 'tsl-film-grain';

export default function TslFilmGrainShowcase() {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const engineRef = useRef<ReturnType<typeof createTslFilmScene> | null>(null);
  const values = useBridgeStore((s) => s.componentValues[BRIDGE_ID]);

  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    const engine = createTslFilmScene(canvas, {});
