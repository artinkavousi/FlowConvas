...
import { createTslPixelationScene } from './TslPixelation.module';

const BRIDGE_ID = 'tsl-pixelation';

export default function TslPixelationShowcase() {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const engineRef = useRef<ReturnType<typeof createTslPixelationScene> | null>(null);
  const values = useBridgeStore((s) => s.componentValues[BRIDGE_ID]);

  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    const engine = createTslPixelationScene(canvas, {});
